...
export default function ContactPage() {
  ...
  const socials = [
    { icon: <Users size={18} />, label: 'LinkedIn', href: 'https://www.linkedin.com/in/dattasable/', color: 'var(--accent)' },
    { icon: <GitBranch size={18} />, label: 'GitHub', href: 'https://github.com/sabledattatray', color: 'var(--muted)' },
    { icon: <MessageCircle size={18} />, label: 'Twitter / X', href: 'https://x.com/sabledattatray', color: 'var(--accent2)' },
    { icon: <Send size={18} />, label: 'Telegram', href: 'https://t.me/sabledatta', color: 'var(--accent3)' },
  ];
  return (
            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }}>
                  ...
                  {socials.map(s => (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl" 
                           style={{ background: `radial-gradient(circle at center, ${s.color}22 0%, transparent 70%)` }} />
              ...
              <div className="card" style={{ padding: '1.5rem', background: 'rgba(255, 255, 255, 0.02)' }}>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button title="Login with Google" onClick={() => signIn('google')} className="flex items-center justify-center h-10 rounded-none border border-white/10 hover:border-white/20 hover:bg-white/5 transition-all">
                  ...
                  <button title="Login with LinkedIn" onClick={() => signIn('linkedin')} className="flex items-center justify-center h-10 rounded-none border border-[#0077b5]/30 hover:border-[#0077b5] hover:bg-[#0077b5]/5 transition-all">
                  ...
                  <button title="Login with GitHub" onClick={() => signIn('github')} className="flex items-center justify-center h-10 rounded-none border border-white/20 hover:border-white/40 hover:bg-white/5 transition-all">
              ...
              <div className="card" style={{ padding: 'clamp(1.5rem, 3vw, 2.5rem)' }}>
                ...
                {status === 'success' && (
                  <motion.div
                    ...
                    style={{ background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)', color: 'var(--accent3)' }}
                  >
                    <CheckCircle size={18} />
                ...
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 250px), 1fr))', gap: '1rem' }}>
                    {[
                      ...
                      <div key={field.name}>
                        <label style={{ display: 'block', color: 'var(--muted)', fontSize: '0.8rem', marginBottom: '0.5rem', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                          {field.label}
                        </label>
                        <input
                          type={field.type}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      style={{
                        width: '100%', padding: '0.75rem 1rem',
                        background: 'var(--tag-bg)', border: '1px solid var(--border)',
                        borderRadius: '8px', color: form.subject ? 'var(--text)' : 'var(--muted)',
                        fontSize: '0.9rem', outline: 'none', fontFamily: 'Space Grotesk, sans-serif',
                      }}
                    >
                      ...
                      <option value="BI Strategy">BI Strategy</option>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      value={form.message}
                      onChange={handleChange}
                      placeholder="Tell me about your project, data challenges, or what you'd like to achieve..."
                      required
                      rows={5}
                      style={{
                        width: '100%', padding: '0.75rem 1rem',
                  ...
                  <button
                    ...
                    style={{ opacity: status === 'loading' ? 0.7 : 1, cursor: status === 'loading' ? 'not-allowed' : 'pointer', position: 'relative', zIndex: 1 }}
                  >
                    {status === 'loading' ? (
                      <>Sending…</>
                    ) : (
